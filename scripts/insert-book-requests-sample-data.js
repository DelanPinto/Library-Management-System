    ...
    try {
        ...
        for (const request of bookRequests) {
            await connection.query(
                `INSERT INTO book_records (user_id, book_id, request_type, status, request_date) VALUES (?, ?, ?, ?, ?)`,
                [request.user_id, request.book_id, request.request_type, request.status, request.request_date]
            );
        }

        console.log('Sample book records data inserted successfully');
    } catch (error) {
        console.error('Error inserting sample book records data:', error);
    } finally {
        if (connection) {
